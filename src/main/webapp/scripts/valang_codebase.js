...
ValangValidator.prototype = {
        ...
        window.onload = function() {
        	...
        	for ( var int = 0; int < fields.length; int++) {
        		...
        		thisField.fieldElement.onblur = function() {
        			// fire any old onblur
        			if(this._oldBlur) {
        	            ValangValidator.Logger.log('Calling OldBlur');
        				this._oldBlur();
        	            ValangValidator.Logger.log('End OldBlur');
        			}
        			// check this field
    ...
    _validateField: function(field) {
        ...
        ValangValidator.Logger.log('Evaluating ' + this.rules.length + ' rules');
        for (var i = 0; i < this.rules.length; i++) {
            var rule = this.rules[i];
            if(rule.field == field.name) {
	            ValangValidator.Logger.log('Evaluating rule for field [' + rule.field + ']');
